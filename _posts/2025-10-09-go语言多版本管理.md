...
- 新增`GOPATH`环境变量，指向go项目的根目录，也就是 `bin, pkg, src`目录
- 新增`GOPROXY`环境变量, 指向`https://goproxy.cn,direct`
- 【已安装过go操作】移除并替换原有go的path 环境变量配置， `go/pathto/bin`，更新为`%USERPROFILE%\.g\go\bin`
- 【已安装过go操作】移除并替换原有的`GOROOT`环境变量配置，更新为`%USERPROFILE%\.g\go`
